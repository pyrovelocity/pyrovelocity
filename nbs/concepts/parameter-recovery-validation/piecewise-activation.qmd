---
title: Piecewise activation with latent time
toc: true
...
This document details the most sophisticated model in the PyroVelocity parameter recovery validation suite: dimensionless analytical dynamics with latent time coordinates and piecewise activation patterns. This model features independent latent temporal coordinates, piecewise constant transcription rates across three phases (off, on, return-to-off), and support for pre-activation scenarios with negative onset times. The framework maintains analytical tractability while supporting rich expression pattern representation and includes comprehensive posterior predictive checking capabilities. This model addresses the most challenging biological scenarios where temporal structure must be inferred from data.
...
The posterior predictive checking procedure provides a rigorous mathematical framework for validating the piecewise activation model's ability to recover known parameters from synthetic data. This approach applies the [general parameter recovery validation framework](index.qmd#general-mathematical-framework-for-parameter-recovery-validation) to the specific case of piecewise activation with latent time coordinates.

For comprehensive mathematical foundations of prior predictive distributions, uncertainty decomposition, Bayesian learning, and validation methodology, see the [general framework documentation](index.qmd#general-mathematical-framework-for-parameter-recovery-validation). This section focuses on the model-specific implementation details and applications.
