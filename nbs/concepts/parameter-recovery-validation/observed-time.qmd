...
abstract: |
  This document presents the dimensionless analytical dynamics model with observed time coordinates for
  parameter recovery validation in PyroVelocity. This model incorporates full transcription-splicing-degradation
  dynamics using analytical solutions where temporal coordinates are treated as observed variables.
  The dimensionless parameterization naturally avoids identifiability issues while maintaining biological
  interpretability through relative degradation rates, characteristic scales, and lumped capture efficiency.
  This model represents the middle complexity tier, bridging simple Poisson fitting and complex latent time models.
