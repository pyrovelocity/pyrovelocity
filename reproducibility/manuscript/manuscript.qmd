...
exist in nature or point to the wrong directions or cannot be recovered due to over-simplified transcription process assumption, as it has been shown in the analysis of fully mature blood cells [@Zheng2017-bz;@Bergen2021-qz;@Ancheta2024.06.25.600667]. Second, data preprocessing steps (e.g.,
...
predictions [@zheng_pumping_2023]. 
In addition, non-linear spliced and unspliced imputation procedures
could distort phase portrait geometry and inferred cell fate [@gorin_rna_2022]
...
interdependence of gene expression. Fourth, we discovered even bigger issues from the inconsistency of the velocity estimates and the correlation of the uncertainty than the interdependence issue [@lederer_statistical_2024]. 
Fifth, a unified description of the temporal relation between cells can only be assigned based on postprocessing steps that aggregate heuristically the gene-wise fits into a shared latent time [@Gu2022.07.08.499381;@gao_unitvelo_2022;@lederer_statistical_2024] ([@fig-model]a). Finally, current RNA velocity frameworks only output point
...
Furthermore, to better recapitulate some dynamic patterns we observed for certain families of genes and  as described in detail later, we extended the original kinetic model by allowing each gene a time lag $(t_{(0,g)}^(0), t_{(0,g)}^(1))$ for a  baseline expression level potentially different than 0 and the transcriptional activation for spliced and unspliced count $((u_{cg} ) ̂, (s_{cg} ) ̂)$  (Model 2, Methods Section Model formulation) (Fig. 1b). By evaluating these two models together with scvelo on X10 datasets, including  differentincluding different tissues and developmental conditions in pancreas, bone marrow, brain pons and peripheral blood cells (see Methods Section Single-cell dataMethods). We discovered consistently better gene expression reconstruction (mean absolute error, Fig. 1d left ) and Cross-boundary directional (CBDir) correctness (Fig. 1d right) for Model 2. The CBDir metric, adapted from the UniTVelo  package [@gao_unitvelo_2022], measures the accuracy of transitions between source and target clusters using boundary cells, reflecting short-term biological development. This metric provides a quantitative assessment of the velocity graph's ability to capture true developmental trajectories. For the remainder of the manuscript we will refer Model 2 as Pyro-Velocity.
...
Although several RNA velocity methods [11–15] have been recently proposed, the goal of this manuscript is not to perform a systematic benchmark of all the available methods, for that the reader may refer to [@Ancheta2024.06.25.600667]. Instead, we want to showcase the advantages and innovations of Pyro-Velocity—namely efficient Bayesian formulation, uncertainty estimation, shared time, and flexible initial conditions, and direct modeling of raw sequencing read count—by comparing it with one of the most used models, i.e., the dynamical RNA velocity model proposed by Bergen et al. and implemented in scVelo.
...
The CBDir metric, adapted from the UniTVelo  package [@gao_unitvelo_2022], measures the accuracy of transitions between source and target clusters using boundary cells, 
...
::: {#fig-model}
![](figures/v2/Figure1.png){width=98%}
...
Velocity and Pyro&thinsp;-Velocity workflows. Both methods take as input spliced
...
visualize uncertainty based on n=30 iterations, posterior samples depicted as
...
activation state and $1$ indicates the repression state. Each gene has two
